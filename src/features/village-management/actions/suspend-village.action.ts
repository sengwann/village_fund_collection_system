...
import { canSuspendVillage } from "../lib/village-lifecycle";
...
export async function suspendVillageAction(formData: FormData): Promise<void> {
  ...
  await withSerializableRetry(() =>
    prisma.$transaction(
      async (tx) => {
        ...
        if (!canSuspendVillage(village.status)) {
        ...
        await tx.village.update({
          ...
          data: {
            status: VillageStatus.SUSPENDED,
          },
        });
        ...
        await createAuditLog(tx, {
          ...
          metadata: {
            ...
            newStatus: VillageStatus.SUSPENDED,
          },
        });
      },
      {
